...
const Hero: React.FC = () => {
  const [pitch, setPitch] = useState<string>("ArgusTech: Transformamos seu negócio local em uma potência digital com sites profissionais que atraem clientes e geram resultados reais.");
  ...
  return (
    <section className="relative min-h-screen pt-32 pb-20 overflow-hidden flex flex-col items-center justify-center">
      ...
      <div className="max-w-7xl mx-auto px-6 text-center mb-16 relative z-10">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-blue-500/30 bg-blue-500/10 text-blue-400 text-[10px] font-black tracking-[0.2em] uppercase mb-8 animate-fade-in">
        ...
        <h1 className="font-montserrat text-5xl md:text-8xl font-black text-white leading-tight tracking-tighter mb-10">
          PRESENÇA <span className="bg-gradient-to-r from-blue-400 via-indigo-300 to-purple-400 bg-clip-text text-transparent">DIGITAL</span>
        </h1>
        
        {/* Pitch Card Container */}
        <div className={`max-w-3xl mx-auto relative transition-all duration-1000 transform ${isLoading ? 'opacity-0 translate-y-4 blur-md' : 'opacity-100 translate-y-0 blur-0'}`}>
        ...
        <div className="mt-14 flex flex-col sm:flex-row items-center justify-center gap-6">
          <a href="#portfolio" className="group flex items-center gap-3 bg-white text-gray-950 px-10 py-4 rounded-full font-bold text-lg hover:bg-blue-600 hover:text-white transition-all transform hover:scale-105 active:scale-95 shadow-xl shadow-white/5">
            Ver Portfólio
            <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
          </a>
          <a href="#contact" className="px-10 py-4 rounded-full font-bold text-lg text-white border border-white/10 hover:bg-white/5 transition-all">
            Solicitar Orçamento
          </a>
        </div>
      </div>
          ...
          {[...PROJECTS, ...PROJECTS].map((project, idx) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-gray-950 via-gray-950/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 p-8 flex flex-col justify-end">
                <span className="text-blue-400 text-[10px] font-black tracking-widest uppercase mb-2">{project.category}</span>
                <h3 className="text-2xl font-bold text-white mb-4">{project.title}</h3>
                <div className="flex gap-2">
                   {project.tags.map(tag => (
                     <span key={tag} className="text-[9px] px-2 py-1 rounded-md bg-white/10 text-white/70 backdrop-blur-sm border border-white/10">
                       {tag}
                     </span>
                   ))}
                </div>
